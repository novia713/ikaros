...
$slug       = $config['config']['list']['slug'];
$owner_name = $config['config']['list']['screen_name_owner'];


//foreach( $config['config']['query'] as $query) {

    $reply = $cb->lists_statuses("&slug=$slug&owner_screen_name=$owner_name&include_rts=true&count=99", true);
	//print_r($reply);die();
...
echo "$ua usuarios insertados, $ta tuits insertados de la lista ". $config['config']['list']['slug']. " \r\n";
?>
